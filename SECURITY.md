...
| `.devcontainer/` | **Local demo only** — NetBox + official Diode quickstart + workspace; not a production deployment template |
...
| Local stack secrets | `.devcontainer/.env.local`, `.devcontainer/diode/.env`, `.devcontainer/netbox/env/*.env`, OAuth JSON | Generated by `.devcontainer/setup.sh` (official Diode quickstart + NetBox env), mode `600`, gitignored |
...
or Codespace. Diode server files come from the upstream `quickstart.sh`; NetBox
with the Diode plugin is composed here. See `.devcontainer/README.md`.

### Controls in place

- Published ports bind to **127.0.0.1** only (`8000` NetBox; Diode nginx
  overridden to loopback in the parent compose file).
- NetBox DB/Redis are not published to the host.
- Secrets are generated with `/dev/urandom` / upstream quickstart, written mode
  `600`, and **preserved** on re-run of `setup.sh` (no silent password rotation
  against live volumes).
