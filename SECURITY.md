...
| `dev/` + `.devcontainer/` | **Local demo only** — NetBox + Diode + workspace; not a production deployment template |
...
| Local stack secrets | `dev/.env.local`, `dev/diode/.env`, `dev/netbox/env/*.env`, OAuth JSON | Generated by `dev/setup.sh`, mode `600`, gitignored |
...
- Dev Container runs `./dev/setup.sh` as host `initializeCommand` before
  compose starts; workspace image tags stay unpinned (rebuild when the
  Dockerfile changes).
- NetBox MCP launcher (`dev/netbox-mcp.sh`) embeds no secrets; it reads
  gitignored `.env.local` and pins the MCP package to a release tag.
...
- Early `dev/netbox/env/*.env` commits used shared demo credentials; those
  files are gitignored at tip. Do not reuse values from history for anything
  beyond a disposable local volume.
...
- [ ] After first local bootstrap, set `BOOTSTRAP: false` in `dev/agent.local.yaml`
