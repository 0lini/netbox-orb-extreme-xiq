...
| SSIDs / WLANs (ConfigState) | `WirelessLAN` — `ssid`, `status` (`active`/`disabled` from SSID enabled), `auth_type` when `encryption` maps cleanly; deduped by SSID across APs (not site-scoped) |
...
| Wireless radios / WLANs | synced | synced (ConfigState wireless + SSID tables → Interface RF fields + WirelessLAN) |
