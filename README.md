...
- **LAG parent** is an `Interface` with `type=lag`, name from `name` (or
  ...
  that interface id apply vlan-properties (untagged / tagged VLANs and
  802.1Q `mode`), PoE (`poe_mode`), and interface IP addresses the same way
  as for physical ports. When AssetPortConfig/State also returns the LAG's
  `asset_interface_id`, description, `mark_connected`, and
