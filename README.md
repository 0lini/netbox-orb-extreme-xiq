...
- **Assets API** (`POST /assets/v1/devices`) — device inventory: hostname,
  ...
  Engine, EXOS, VOSS, AP, …) that gates the port sync and becomes Device
  role when present (never a static default).
...
one call per device-filtered port/LAG/capabilities/PoE-state table covering
...
real value: the role **name** is the function string as reported (e.g.
`Fabric Engine`, `AP`), and the **slug** is a lowercase hyphenated form of
that string (`fabric-engine`, `ap`). Empty / missing function, the Assets
sentinel `Unknown`, or a value that cannot form a valid slug assert **no**
role — there is no static default (`switch`, `network`, `unknown`, …). Diode
treats `Device.role` as optional, so omitting it leaves role NetBox-owned
(same Assurance posture as omitting `Interface.type` when the connector
code is unverified). NetBox's UI still requires a role for manually created
devices; that does not force the worker to invent one.
...
- **VLANs and 802.1Q mode** prefer
  ...
  VLAN. VLANs are referenced by bare `vid`; no VLAN names or groups are
  asserted.
